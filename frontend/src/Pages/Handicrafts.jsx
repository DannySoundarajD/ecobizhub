...
import { Star, Filter, TreePine, ShoppingCart, Heart, ChevronLeft, ChevronRight, Loader, Search, X, SlidersHorizontal, Leaf, IndianRupee, RotateCcw, User, Tag, ShieldCheck } from 'lucide-react';
...
const Handicrafts = () => {
    ...
    const COLLECTION_NAME = 'handicrafts';
    ...
    const filteredAndSortedProducts = useMemo(() => {
        ...
        if (debouncedSearchTerm) {
            ...
            currentProducts = currentProducts.filter(product =>
                ['name', 'description', 'material', 'origin', 'type'].some(field =>
                    product[field]?.toLowerCase().includes(searchLower)
                )
            );
        }
        if (selectedMaterial !== 'All') currentProducts = currentProducts.filter(p => p.material === selectedMaterial);
    ...
    const uniqueMaterials = useMemo(() => ['All', ...new Set(products.map(p => p.material).filter(Boolean))], [products]);
    ...
    const toggleWishlist = useCallback(async (product, e) => {
        ...
        try {
            ...
            if (!snapshot.empty) {
                ...
                await addDoc(collection(db, 'wishlists'), { userId: user.uid, productId, productData: product, addedAt: serverTimestamp(), category: 'handicrafts' });
            }
    ...
    const addToCart = useCallback(async (product, e, quantity = 1) => {
        ...
        try {
            ...
            if (!snapshot.empty) {
                ...
                await addDoc(collection(db, 'cart'), { userId: user.uid, productId, quantity, productData: product, addedAt: serverTimestamp(), updatedAt: serverTimestamp(), category: 'handicrafts' });
            }
            ...
            <div className="bg-gradient-to-r from-green-600 to-lime-600 text-white">
                <div className="container mx-auto px-4 py-16 text-center">
                    <div className="flex items-center justify-center gap-2 mb-4">
                        ...
                        <h1 className="text-4xl md:text-5xl font-bold">Handicrafts</h1>
                        <TreePine className="w-8 h-8" />
                    </div>
                    <p className="text-xl text-green-100 max-w-2xl mx-auto">Discover unique, handmade crafts that support local artisans and sustainable practices.</p>
                </div>
            </div>
                                ...
                                {paginatedProducts.map((product) => {
                                    ...
                                    return (
                                        <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group" key={productId}>
                                            <Link to={`/handicrafts/${productId}`} className="block">
